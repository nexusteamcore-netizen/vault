...
const os = require('os');
const CONFIG_PATH = path.join(os.homedir(), '.phantomapi.json');
...
async function run() {
  ...
  if (command === 'login') {
    const key = args[1];
    if (!key) {
      console.error('\x1b[31m[Error]\x1b[0m Please provide your API key: phantomapi login pa_your_key');
      process.exit(1);
    }

    const newConfig = { ...config, apiKey: key };
    
    // Auto-detect URL from smart token pa_BASE64URL_HASH
  ...
  if (command === 'config') {
    ...
    const val = args[2];
    if (!key || !val) {
      console.log('\x1b[33mUsage:\x1b[0m phantomapi config <key> <value>');
  ...
  if (!command) {
    console.error(`
\x1b[36m--- PhantomAPI CLI v1.0.2 ---\x1b[0m
Steps to get started:
1. \x1b[33mphantomapi login <API_KEY> [URL]\x1b[0m  (Authenticates your machine)
2. \x1b[33mphantomapi list\x1b[0m                  (Shows your secrets)
3. \x1b[33mphantomapi get <NAME>\x1b[0m            (Retrieves a secret)

Commands:
  phantomapi login <KEY> [URL]
  phantomapi get <NAME>
  phantomapi list
  phantomapi set <NAME> <VALUE>
  phantomapi delete <NAME>
  phantomapi config baseUrl <URL>
`);
    process.exit(1);
  }

  const { apiKey, baseUrl: configUrl } = loadConfig();
  const token = process.env.PHANTOMAPI_API_KEY || apiKey;
  const baseUrl = process.env.PHANTOMAPI_URL || configUrl || 'http://localhost:3000';

  if (!token) {
    console.error(`\x1b[31m[Error]\x1b[0m No API key found.`);
    console.log('\x1b[33mTip:\x1b[0m Run "phantomapi login <key>" to authenticate.');
    process.exit(1);
  }
  ...
  const callApi = async (path, method = 'GET', body = null) => {
    try {
      ...
      throw new Error(`PhantomAPI API Error: ${err.message}`);
    }
  };
  ...
  if (command === 'get') {
    ...
    const value = args[2];
    if (!target || !value) {
      console.error('\x1b[31m[Error]\x1b[0m Usage: phantomapi set <NAME> <VALUE>');
      process.exit(1);
    }
